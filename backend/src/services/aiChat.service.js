    ...
    static deepMerge(existing, newData) {
        ...
        if (newData.experience && newData.experience.length > 0) {
            newData.experience.forEach(newExp => {
                if (newExp.title || newExp.company) {
                    const existingIndex = result.experience.findIndex(e =>
                        e.company === newExp.company && e.title === newExp.title
                    );

                    if (existingIndex >= 0) {
                        result.experience[existingIndex] = { ...result.experience[existingIndex], ...newExp };
                    } else {
        ...
        if (newData.education && newData.education.length > 0) {
            newData.education.forEach(newEdu => {
                if (newEdu.institution || newEdu.degree) {
                    const existingIndex = result.education.findIndex(e =>
                        e.institution === newEdu.institution && e.degree === newEdu.degree
                    );

                    if (existingIndex >= 0) {
                        result.education[existingIndex] = { ...result.education[existingIndex], ...newEdu };
                    } else {
        ...
        if (newData.key_projects_achievements && newData.key_projects_achievements.length > 0) {
            newData.key_projects_achievements.forEach(newProj => {
                if (newProj.name) {
                    const existingIndex = result.key_projects_achievements.findIndex(p =>
                        p.name === newProj.name
                    );

                    if (existingIndex >= 0) {
                        result.key_projects_achievements[existingIndex] = { ...result.key_projects_achievements[existingIndex], ...newProj };
                    } else {
...
Be natural and conversational. Don't ask for things the user already provided.`;
    ...
    static getFollowUpQuestion(field, currentData) {
        const questions = {
            personal_info: "ask for their full name, professional title, and contact information (email, phone, location, LinkedIn profile if they have one).",
            summary: "ask them to describe their professional background in 2-3 sentences - what they do, their experience level, and what they're looking for.",
            experience: "ask about their work history. Request details about their most recent or current position: job title, company name, location, employment dates (start and end), and 2-3 key achievements or responsibilities as bullet points.",
            education: "ask about their educational background - degree obtained, institution name, field of study, and graduation date.",
            skills: "ask about their skills. What are their core competencies? What software, tools, or technologies are they proficient in? Any languages they speak? Professional certifications?",
            key_projects_achievements: "ask if they have any notable projects or achievements to highlight. What was the project, what did they accomplish, and what skills or technologies did they use?",
            awards_and_recognitions: "ask if they have received any awards, honors, or professional recognitions worth mentioning."
        };
